...
export class PrintProjectLedgerComponent extends Component {
  render() {
    const { id,filterValues } = this.props;
    return <ProjectLedegerReport coa={id} filter={filterValues} />;
  }
}
...
export const ProjectLedegerReport = (props) => {
  ...
  const { data, loaded, error } = useQueryWithStore({
    ...
    payload: {
      ...
      filter: { project: jwt.project.id, filter:props.filter },
    },
  });
          ...
          <div className="invoice-box">
            <div>
              <h2 className="report-title">
                {" "}
                {records[0] && records[0].Project_Title}<br />
                <span class="title_ledger">Account Ledger</span>
              </h2>
            </div>
            <div class="heading-title">
              <h2 style={{ margin: "15px" }}>
                ...
                <span class="obal">
                  Opening Balance : {formatCurrency(records[0] && records[0].coa_obal || 0)}
                </span>
              </h2>
            </div>
                  ...
                  if (record.id) {
                    return (
                      <tr>
                        ...
                        <td className="vou_date">{record.vou_date_string}</td>
                        <td className="description">{record.Particulars}</td>
...
const ProjectLedgerPrintable = (props) => {
  ...
  console.log("RRRRRRRRRRRRRRRRRRR" + JSON.stringify(props));
  return (
    <div>
...
const ProjectLedgerListActions = (props) => (
  <TopToolbar>
...
const ProjectLedgerDetailButton = (props) => {

  const listContext = useListContext();
  //console.log("record" + JSON.stringify(record));
  //const mbasepath = "/reports/projectledger/" + JSON.stringify(filter.filter);
 return ( <Button
            ...
            to={{
              state: {
                record: {
                  id: props.record.id,
                  myrange:listContext.filterValues
                },
              },
            }}
            label="Show Detail"
          ...
          />);
  //return (
    //<EditButton basePath={mbasepath} label="Edit comment" record={record} />
  //);
};

export const ProjectLedgerList = (props) => {
  console.log("list:" + JSON.stringify(props.location.search));
  return (<div>
    {/* <MyProjectLedgerPrint {...props}></MyProjectLedgerPrint> */}
    <List
      aside={<FilterSidebar />}
      perPage={1000}
      actions={<ProjectLedgerListActions />}
    ...
    >
      {
        <Datagrid rowClick="edit" >
          ...
          {/* <ProjectLedgerDetailButton props={props}/>
          <Button
          
            component={Link}
            to={{
              pathname: '/projectledger',
              search: `filter=${JSON.stringify({ id: props.ids })}`,
              state: {
                record: {
                  myrange:JSON.stringify(props.location.search),
                },
              },
            }}
            label="Show Detail"
...
const MyProjectLedgerPrint=(props)=>{
  const {data} =useListController(props);
  //console.log("project leder :"+JSON.stringify(useListController(props)));
  return(
    JSON.stringify(data)
  );
}
