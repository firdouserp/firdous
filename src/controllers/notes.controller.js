...
class NotesController {
    getAllNotes = async (req, res, next) => {
        ...
        if (req.query && Object.keys(req.query).length) {
             range = req.query.range && JSON.parse(req.query.range);
             sort = req.query.sort && JSON.parse(req.query.sort);
             filter = range && req.query.filter && JSON.parse(req.query.filter);
            //console.log(range)
        ...
        // }
        notesList = await NotesModel.find(filter, range, sort,req.projectid);


        let count = await NotesModel.count(filter,req.projectid);
    ...
    ListAllNotes = async (req, res, next) => {
        ...
        let notesList = await NotesModel.list(req.projectid);
    ...
    getNotesByNotesName = async (req, res, next) => {
        const Notes = await NotesModel.findOne({ notesname: req.params.notesname },req.projectid);
        if (!notes) {
    ...
    createNotes = async (req, res, next) => {
        ...
        const result = await NotesModel.create(req.body,req.projectid);
