...
class CoaController {
  getAllCoa = async (req, res, next) => {
    ...
    if (req.query && Object.keys(req.query).length) {
      ...
      //console.log(range)
      // coaList = await CoaModel.find(filter, range, sort,req.projectid);
     } 
    //  else {
      coaList = await CoaModel.find(filter, range, sort,req.projectid);
    // }

    let count = await CoaModel.count(filter,req.projectid);
  ...
  createCoa = async (req, res, next) => {
    ...
    const result = await CoaModel.create(req.body,req.projectid);
  ...
  autocomplete = async (req, res, next) => {
    ...
    if (req.query && Object.keys(req.query).length) {
      ...
      // coaList = await CoaModel.autocomplete(filter, sort,pro);
    } 
    // else {
    //   coaList = await CoaModel.autocomplete();
    // }
    coaList = await CoaModel.autocomplete(filter, sort,req.projectid);
